...
def generate_social_random_walk_sequence(data_path:str, num_nodes:int=10, walk_length:int=5, save_flag=False, all_node=False, seed=False) -> dict:
    """
    ...
    if save_flag:
        # save result to .csv
        path = data_path + '/' + f"social_user_{num_nodes}_rw_length_{walk_length}_seed_.csv"
        columns = ['user_id', 'random_walk_seq']
        result_df = pd.DataFrame(path_dict.items(), columns=columns)
...
# For checking
sequences = generate_social_random_walk_sequence(data_path, num_nodes=10, walk_length=20, save_flag=False, all_node=False, seed=False)
